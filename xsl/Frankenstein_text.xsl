<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:tei="http://www.tei-c.org/ns/1.0"
    exclude-result-prefixes="xs tei"
    version="2.0">
    
    <!-- <xsl:output method="xml" omit-xml-declaration="yes" indent="yes" /> -->
    <xsl:template match="tei:teiHeader"/>

    <xsl:template match="tei:body">

        <div class="row">
        <div class="col-3"><br/><br/><br/><br/><br/>
            <xsl:for-each select="//tei:add[@place = 'marginleft']">
                <xsl:choose>
                    <xsl:when test="parent::tei:del">
                        <del>
                            <xsl:attribute name="class">
                                <xsl:value-of select="attribute::hand" />
                            </xsl:attribute>
                            <xsl:value-of select="."/></del><br/>
                    </xsl:when>
                    <xsl:otherwise>
                        <span >
                        <i>
                            <xsl:attribute name="class">
                                <xsl:value-of select="attribute::hand" />
                            </xsl:attribute>
                            <xsl:value-of select="."/><br/></i></span>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:for-each> 

        </div>
        <div class="col-9">
            <div class="transcription">
                <xsl:apply-templates select="//tei:div"/> 
            </div>
        </div>
        </div> 
    </xsl:template>


    <xsl:template match="tei:div">
        <div class="#MWS">  
            <xsl:apply-templates/>
        </div>
    </xsl:template>
    
    <xsl:template match="tei:p">
        <p>
            <xsl:apply-templates/>
        </p>
    </xsl:template>

    <xsl:template match="tei:add[@place = 'marginleft']">
        <span class="marginAdd">
            <xsl:apply-templates/>
        </span>
    </xsl:template>
    
    <xsl:template match="tei:del">
        <del>
            <xsl:attribute name="class">
                <xsl:value-of select="@hand"/>
            </xsl:attribute>
            <xsl:apply-templates/>
        </del>
    </xsl:template>
    
    <xsl:template match="@*|node()">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
    </xsl:template>

    <!-- I had to get rid of the class supraAdd - it was conflicting with the class element taking the hand value-->
    <xsl:template match="tei:add[@place = 'supralinear']">
        <sup><i>
                <xsl:attribute name="class">
                    <xsl:value-of select="@hand"/>
                </xsl:attribute>
                <xsl:apply-templates/>
        </i></sup>
    </xsl:template>

 <!--italicised inline additions-->
    <xsl:template match="tei:add[@place = 'inline']">
        <i>
            <xsl:attribute name="class">
                <xsl:value-of select="@hand"/>
            </xsl:attribute>
            <xsl:apply-templates/>
        </i>
    </xsl:template>
 
    <!-- add additional templates below, for example to transform the tei:lb in <br/> empty elements, tei:hi[@rend = 'sup'] in <sup> elements, the underlined text, additions with the attribute "overwritten" etc. -->
    
    <!--line breaks-->
    <xsl:template match="tei:lb">
        <br/>
    </xsl:template>
    
    <!--circled-->
    <xsl:template match="tei:hi[@rend='circled']">
        <span class="circled-text">
            <xsl:apply-templates/>
        </span>
    </xsl:template>

    <!-- overwritten additions -->
    <xsl:template match="tei:add[@place='overwritten']">
        <strong>
            <i>
                <xsl:attribute name="class">
                    <xsl:value-of select="@hand"/>
                </xsl:attribute>
                <xsl:apply-templates/>
            </i>
        </strong>
    </xsl:template>

    <!--superscript-->
    <xsl:template match="tei:hi[@rend='sup']//text()">
        <sup>
            <xsl:value-of select="."/>
        </sup>
    </xsl:template>

<!--underline-->
    <xsl:template match="tei:hi[@rend='u']">
        <u>
            <xsl:apply-templates/>
        </u>
    </xsl:template>  

<!--circled page number-->
    <xsl:template match="tei:metamark[@function='pagenumber']">
        <p class="align-right">
             <xsl:apply-templates/>
        </p>
    </xsl:template>

<!-- comments by me smaller, in italic and in grey-->
    <xsl:template match="tei:note[@resp='#TB']">
        <div class="note">
             <xsl:apply-templates/>
        </div>
    </xsl:template>

<!-- metamarks in brown -->
    <xsl:template match="tei:metamark">
        <span class="metamark">
            <xsl:apply-templates/>
        </span>
    </xsl:template>

    <!--sic and corr-->
    <xsl:template match="tei:sic">
        <span class="sic">
            <xsl:apply-templates/>
        </span>
    </xsl:template>

    <xsl:template match="tei:corr">
        <span class="corr">
            <xsl:apply-templates/>
        </span>
    </xsl:template>

<!--paragrahps with no indentation-->
    <xsl:template match="tei:p[@class='no-indent']">
        <p class="no-indent">
            <xsl:apply-templates/>
        </p>
    </xsl:template>

<!--full paragraph indented to the right - look into this!!!-->
    <xsl:template match="tei:p[@class='right']">
        <p class="indent-p">
            <xsl:apply-templates/>
        </p>
    </xsl:template>

</xsl:stylesheet>